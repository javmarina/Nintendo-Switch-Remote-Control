...
uint8_t button_color[] = {0xDD, 0xDD, 0xDD}; // Gray
uint8_t left_grip_color[] = {0xAA, 0xAA, 0xAA};
uint8_t right_grip_color[] = {0xAA, 0xAA, 0xAA};
...
static void prepare_spi_reply(uint16_t address, uint8_t data[], uint8_t length) {
    ...
    spiReplyBuffer[4] = length;
    memcpy(&spiReplyBuffer[5], &data[0], length);
...
void spi_read(SPI_Address_t address, size_t size) {
    uint8_t buf[size];
    ...
    prepare_spi_reply(address, buf, size);
}
